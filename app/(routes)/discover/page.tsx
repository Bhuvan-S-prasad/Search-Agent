...
function Discover() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      {/* HEADER */}
      <div className="border-b border-border mt-17 ml-17">
          ...
          <div className="flex gap-3">
            {options.map((option, index) => (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-full hover:bg-sidebar-accent hover:text-slate-600 ${
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="grid grid-cols-12 gap-8">
              ...
              <div className="space-y-8">
                ...
                {latestNews[4] && (
                  <article className="group cursor-pointer border-t border-border pt-6">
                    <a href={latestNews[4].link} target="_blank" rel="noopener noreferrer" className="flex gap-6">
                ...
                {latestNews.slice(5).reduce((acc, news, i) => {
                  ...
                  if (indexInGroup < 2) {
                    ...
                    if (indexInGroup === 0) {
                      acc.push(
                        <div key={`grid-${i}`} className="grid grid-cols-2 gap-6 border-t border-border pt-6">
                          <article className="group cursor-pointer">
                            <a href={news.link} target="_blank" rel="noopener noreferrer">
                              <div className="relative h-40 mb-3 rounded-lg overflow-hidden">
                                <Image
                    ...
                    acc.push(
                      <article key={`full-${i}`} className="group cursor-pointer border-t border-border pt-6">
                        <a href={news.link} target="_blank" rel="noopener noreferrer" className={`flex gap-6 ${imageOnRight ? 'flex-row-reverse' : ''}`}>
                          <div className="relative w-48 h-32 flex-shrink-0 rounded-lg overflow-hidden">
                            <Image
                              src={news.image}
                              alt={news.title}
          ...
          <div className="col-span-4 space-y-8">
            {/* MARKET OUTLOOK */}
            <div>
              <h3 className="text-xl font-bold mb-4">Market Outlook</h3>
              {marketsLoading ? (
                    ...
                    <div key={i} className="pb-4 border-b border-border last:border-0">
                      <div className="flex justify-between items-start mb-1">
                        ...
                        <div className="text-right">
                          <div className="text-lg font-bold">${market.price}</div>
                        </div>
                      </div>
                      <div className={`text-sm font-medium ${parseFloat(market.change) >= 0 ? 'text-green-600 dark:text-green-500' : 'text-red-600 dark:text-red-500'}`}>
                        {parseFloat(market.change) >= 0 ? '+' : ''}{market.change} ({parseFloat(market.changePercent) >= 0 ? '+' : ''}{market.changePercent}%)
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {/* TRENDING COMPANIES - NOW USING LIVE DATA */}
            <div>
              <h3 className="text-xl font-bold mb-4">Trending Companies</h3>
              {marketsLoading ? (
                <div className="flex items-center justify-center py-8">
                  <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>
                </div>
                ...
                <div className="space-y-1">
                  {markets.map((company, i) => {
                    const isPositive = parseFloat(company.change) >= 0;
                    return (
                      <div key={i} className="flex items-center gap-3 py-3 hover:bg-accent/50 transition-colors cursor-pointer rounded-lg px-2 -mx-2">
                        <div className="w-10 h-10 bg-muted rounded-full flex items-center justify-center text-xs font-bold">
                          {company.symbol.slice(0, 2)}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="text-sm font-medium truncate">
                            {COMPANY_NAMES[company.symbol] || company.symbol}
                          </div>
                          <div className="text-xs text-muted-foreground">{company.symbol}</div>
                        </div>
                        <div className="text-right flex-shrink-0">
                          <div className="text-sm font-bold">${company.price}</div>
                          <div className={`text-xs flex items-center justify-end gap-1 ${isPositive ? 'text-green-600 dark:text-green-500' : 'text-red-600 dark:text-red-500'}`}>
